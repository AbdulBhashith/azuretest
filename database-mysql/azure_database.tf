...
resource "azurerm_mysql_firewall_rule" "demo-allow-treck" {
  name                = "mysql-treck"
  resource_group_name = azurerm_resource_group.demo.name
  server_name         = azurerm_mysql_server.demo.name
  start_ip_address    = azurerm_network_interface.treck.private_ip_address
  end_ip_address      = azurerm_network_interface.treck.private_ip_address
}
